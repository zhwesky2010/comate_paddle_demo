...
def mix(hp, args, audio, num, s1_dvec, s1_target, s2, train):
    ...
    assert len(tuple(d.shape)) == len(tuple(w1.shape)) == len(tuple(w2.shape)
        ) == 1, 'wav files must be mono, not stereo'
    d, _ = librosa.effects.trim(d, top_db=20)
    w1, _ = librosa.effects.trim(w1, top_db=20)
    w2, _ = librosa.effects.trim(w2, top_db=20)
    if tuple(d.shape)[0] < 1.1 * hp.embedder.window * hp.audio.hop_length:
        return
    if args.vad == 1:
        w1, w2 = vad_merge(w1), vad_merge(w2)
    L = int(srate * hp.data.audio_len)
    if tuple(w1.shape)[0] < L or tuple(w2.shape)[0] < L:
        return
    w1, w2 = w1[:L], w2[:L]
    mixed = w1 + w2
    ...
    paddle.save(obj=paddle.to_tensor(data=target_mag), path=target_mag_path)
    paddle.save(obj=paddle.to_tensor(data=mixed_mag), path=mixed_mag_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--libri_dir', type=str, default=None, help=
    ...
    parser.add_argument('-o', '--out_dir', type=str, required=True, help=
        'Directory of output training triplet')
    parser.add_argument('-p', '--process_num', type=int, default=None, help
        ='number of processes to run. default: cpu_count')
    parser.add_argument('--vad', type=int, default=0, help=
    ...
    if args.libri_dir is not None:
        train_folders = [x for x in glob.glob(os.path.join(args.libri_dir,
            'train-clean-100', '*')) if os.path.isdir(x)] + [x for x in
            glob.glob(os.path.join(args.libri_dir, 'train-clean-360', '*')) if
            os.path.isdir(x)]
        test_folders = [x for x in glob.glob(os.path.join(args.libri_dir,
            'dev-clean', '*'))]
    ...
    train_spk = [glob.glob(os.path.join(spk, '**', hp.form.input),
        recursive=True) for spk in train_folders]
    train_spk = [x for x in train_spk if len(x) >= 2]
    test_spk = [glob.glob(os.path.join(spk, '**', hp.form.input), recursive
        =True) for spk in test_folders]
