...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--base_dir', type=str, default='.', help=
        'Root directory of run.')
    ...
    parser.add_argument('-m', '--model', type=str, required=True, help=
        'Name of the model. Used for both logging and saving checkpoints.')
    ...
    chkpt_path = (args.checkpoint_path if args.checkpoint_path is not None else
        None)
    logging.basicConfig(level=logging.INFO, format=
        '%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.
        FileHandler(os.path.join(log_dir, '%s-%d.log' % (args.model, time.
        time()))), logging.StreamHandler()])
    logger = logging.getLogger()
    if hp.data.train_dir == '' or hp.data.test_dir == '':
        logger.error('train_dir, test_dir cannot be empty.')
        raise Exception('Please specify directories of data in %s' % args.
            config)
